...
type RestOf<T extends IEmpleado | NewEmpleado> = Omit<T, 'fecIngreso' | 'fecNac' | 'fecCrea' | 'fecModif'> & {
...
export class EmpleadoService {
  ...
  getEmpleadoIdentifier(empleado: Pick<IEmpleado, 'id'>): string {
    return empleado.id;
  }
  ...
  protected convertDateFromClient<T extends IEmpleado | NewEmpleado | PartialUpdateEmpleado>(empleado: T): RestOf<T> {
    return {
      ...empleado,
      fecIngreso: empleado.fecIngreso?.toJSON() ?? null,
      fecNac: empleado.fecNac?.toJSON() ?? null,
      fecCrea: empleado.fecCrea?.toJSON() ?? null,
      fecModif: empleado.fecModif?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmpleado: RestEmpleado): IEmpleado {
    return {
      ...restEmpleado,
      fecIngreso: restEmpleado.fecIngreso ? dayjs(restEmpleado.fecIngreso) : undefined,
      fecNac: restEmpleado.fecNac ? dayjs(restEmpleado.fecNac) : undefined,
      fecCrea: restEmpleado.fecCrea ? dayjs(restEmpleado.fecCrea) : undefined,
      fecModif: restEmpleado.fecModif ? dayjs(restEmpleado.fecModif) : undefined,
    };
  }
